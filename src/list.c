...
/*
 * Ordena la lista con el agoritmo quicksort.
 * comparer: función que compara dos elementos de la lista para ordenarlos.
 * 			si la función devuelve un nro > 0 el segundo elemento es menor
 */
void list_quick_sort(t_list *list, int(*comparer)(void*, void*)){

	void quick_sort(int initialPos, int finalPos){
		int sense=-1, down=initialPos, up=finalPos;
		t_link_element *downElement, *upElement;
		...
		int compare(){
			downElement = list_get_element(list, down);
			upElement = list_get_element(list, up);
			return comparer(downElement->data, upElement->data);
		}

		void swap(){
			void * downElementData =list_replace(list, down, upElement->data);
			list_replace(list, up, downElementData);
		}

		for(;down != up;){
			if(compare() > 0){ // up<down
				swap();
				sense = sense * -1; //cambiar sentido;
				moveNext();
			}
			else{
				moveNext();
		...
		// down == up
		if(down > initialPos)
			quick_sort(initialPos, --down);
		if(up < finalPos)
			quick_sort(++up, finalPos);
	}

	quick_sort(0, list->elements_count-1);
}
